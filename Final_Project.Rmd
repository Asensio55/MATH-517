...
Firstly, the span approach (loess, loess.as) seems to consistently underfit for small data sets, even though the hyperparameter is algorithmically chosen. They work however well for larger data sets. Maybe this is due to the algorithm in loess.as not producing optimal results, this could be checked by computing the span parameter externally. Given the same parameters, both functions produce identical results, meaning loess is better when we already have a prior idea of the span we want to use, whereas loess.as computes a candidate when we have no idea.  
Regarding the bandwidth approach (locpol, locpoly), both functions yield different results when given identical inputs. In this case, we didn't choose the bandwidth parameter optimally, but both seem to work well when given a reasonable estimate.  Locpol is more sensitive to small variations in the data, but thus has a tendency to overfit and is not applicable on larger datasets. Locpoly seems to underfit a bit when given the same bandwidth as locpol, but this can be solved by reducing this parameter, as shown in the previous part. In summary, locpol is better when we think our data is not too noised and we want to detect finer deviations, and locpoly is better otherwise.  
Comparing both approaches differently, we see the span is worse for small datasets (tends to underfit), but when considering larger datasets, both approaches give the same results, in term of efficiency.

Computationnallywise, without surprise the loess.as function is slower than its counterparts, as it calculates its hyperparameter value internally. A relevant comparison would be to run a bandwidth- or span selection algorithm, such as K-folds, and integrating it in the timing of the other functions, to put them on even ground. Loess and locpol are faster, but locpoly is definitely the most efficient in its calculations, partially due to the fact that locpoly only outputs the fit (X,Y coordinates) and nothing else, which is not true for the others.

Altogether, locpoly seems to be a good choice, both in performance and in time. However, we do need to select a bandwidth a priori. Loess and loess.as are basically the same function, thus it all comes down, as said previously, to whether or not we think we have a good span parameter input or not. Locpol is somewhere at the same level of interest; it is slower than locpoly, is not applicable to large datasets, but it gives interesting results and, again, is a more sensitive function to local variations than locpoly. 

# Conclusion

When only dealing with the application of a local regression, the locpoly function from the Kernsmooth package is a great candidate, but the other ones are also decent candidates, due to their versatility (more specific outputs) and their results that are altogether quite efficient.

To look into the matter further, one could explore the following ideas:  

+ Run a K-folds algorithm for choosing bandwidth/span externally, then compare the resulting runtime with loess.as's that does it internally. 
+ Evaluate the performance of the different functions on datasets with a varying X-axis density: here, the data points were evenly distributed on the interval, but variations can have an impact, especially on the bandwidth approach. 
+ Check the code of the locpol and locpoly functions to examine why their results differ
+ Use, instead of the $R^2$ criteria,  a similar version of the calculations but using the residuals from the regression values to the function before it was noised, aka the underlying curve, to assess the performance in a more proper way. Here, the resulting values would decrease as soon as the considered algorithm would start either to underfit, or to overfit, which would make for a relevant complementary criteria to assessing the resulting curve. 
