...
To do so, we first add a 'decenny' variable representing in what decade the film came out. In doing so, we lose some precision over the results in hope to gain a better insight of the temporal evolution. 
```{r decenny, echo = FALSE}
movies_metadata$release_decenny=1:length(movies_metadata$release_date)
for (i in 1:length(movies_metadata$release_date)) {
  if (grepl("190",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1900
  }
  else if (grepl("191",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1910
  }
 else if (grepl("192",movies_metadata$release_date[i]) == TRUE) {
   movies_metadata$decenny[i] = 1920
 }
  else if (grepl("193",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1930
  }
  else if (grepl("194",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1940
    }
  else if (grepl("195",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1950
  }
  else if (grepl("196",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1960
  }
  else if (grepl("197",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1970
  }
  else if (grepl("198",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1980
  }
  else if (grepl("199",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 1990
  }
  else if (grepl("200",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 2000
  }
  else if (grepl("201",movies_metadata$release_date[i]) == TRUE) {
    movies_metadata$decenny[i] = 2010
  }
  else {
    movies_metadata$decenny[i] = 2020
  }
}
```
``` {r runtime, echo = FALSE}

ggplot(movies_metadata, mapping= aes(decenny,vote_average)) + 
     geom_point() + 
     stat_smooth(method = lm) ;

 ggplot(movies_metadata, mapping= aes(decenny,runtime)) + 
     geom_point() + 
     stat_smooth(method = lm) 
```
